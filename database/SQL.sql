USE [master]
GO
/****** Object:  Database [GEOMEDIA]    Script Date: 06/05/2026 09:41:45 ******/
CREATE DATABASE [GEOMEDIA]
 CONTAINMENT = NONE
 ON  PRIMARY 
( NAME = N'GEOMEDIA', FILENAME = N'/var/opt/mssql/data/GEOMEDIA.mdf' , SIZE = 270336KB , MAXSIZE = UNLIMITED, FILEGROWTH = 65536KB )
 LOG ON 
( NAME = N'GEOMEDIA_log', FILENAME = N'/var/opt/mssql/data/GEOMEDIA_log.ldf' , SIZE = 270336KB , MAXSIZE = 2048GB , FILEGROWTH = 65536KB )
 WITH CATALOG_COLLATION = DATABASE_DEFAULT, LEDGER = OFF
GO
ALTER DATABASE [GEOMEDIA] SET COMPATIBILITY_LEVEL = 150
GO
IF (1 = FULLTEXTSERVICEPROPERTY('IsFullTextInstalled'))
begin
EXEC [GEOMEDIA].[dbo].[sp_fulltext_database] @action = 'enable'
end
GO
ALTER DATABASE [GEOMEDIA] SET ANSI_NULL_DEFAULT OFF 
GO
ALTER DATABASE [GEOMEDIA] SET ANSI_NULLS OFF 
GO
ALTER DATABASE [GEOMEDIA] SET ANSI_PADDING OFF 
GO
ALTER DATABASE [GEOMEDIA] SET ANSI_WARNINGS OFF 
GO
ALTER DATABASE [GEOMEDIA] SET ARITHABORT OFF 
GO
ALTER DATABASE [GEOMEDIA] SET AUTO_CLOSE OFF 
GO
ALTER DATABASE [GEOMEDIA] SET AUTO_SHRINK OFF 
GO
ALTER DATABASE [GEOMEDIA] SET AUTO_UPDATE_STATISTICS ON 
GO
ALTER DATABASE [GEOMEDIA] SET CURSOR_CLOSE_ON_COMMIT OFF 
GO
ALTER DATABASE [GEOMEDIA] SET CURSOR_DEFAULT  GLOBAL 
GO
ALTER DATABASE [GEOMEDIA] SET CONCAT_NULL_YIELDS_NULL OFF 
GO
ALTER DATABASE [GEOMEDIA] SET NUMERIC_ROUNDABORT OFF 
GO
ALTER DATABASE [GEOMEDIA] SET QUOTED_IDENTIFIER OFF 
GO
ALTER DATABASE [GEOMEDIA] SET RECURSIVE_TRIGGERS OFF 
GO
ALTER DATABASE [GEOMEDIA] SET  ENABLE_BROKER 
GO
ALTER DATABASE [GEOMEDIA] SET AUTO_UPDATE_STATISTICS_ASYNC OFF 
GO
ALTER DATABASE [GEOMEDIA] SET DATE_CORRELATION_OPTIMIZATION OFF 
GO
ALTER DATABASE [GEOMEDIA] SET TRUSTWORTHY OFF 
GO
ALTER DATABASE [GEOMEDIA] SET ALLOW_SNAPSHOT_ISOLATION OFF 
GO
ALTER DATABASE [GEOMEDIA] SET PARAMETERIZATION SIMPLE 
GO
ALTER DATABASE [GEOMEDIA] SET READ_COMMITTED_SNAPSHOT OFF 
GO
ALTER DATABASE [GEOMEDIA] SET HONOR_BROKER_PRIORITY OFF 
GO
ALTER DATABASE [GEOMEDIA] SET RECOVERY FULL 
GO
ALTER DATABASE [GEOMEDIA] SET  MULTI_USER 
GO
ALTER DATABASE [GEOMEDIA] SET PAGE_VERIFY CHECKSUM  
GO
ALTER DATABASE [GEOMEDIA] SET DB_CHAINING OFF 
GO
ALTER DATABASE [GEOMEDIA] SET FILESTREAM( NON_TRANSACTED_ACCESS = OFF ) 
GO
ALTER DATABASE [GEOMEDIA] SET TARGET_RECOVERY_TIME = 60 SECONDS 
GO
ALTER DATABASE [GEOMEDIA] SET DELAYED_DURABILITY = DISABLED 
GO
ALTER DATABASE [GEOMEDIA] SET ACCELERATED_DATABASE_RECOVERY = OFF  
GO
EXEC sys.sp_db_vardecimal_storage_format N'GEOMEDIA', N'ON'
GO
ALTER DATABASE [GEOMEDIA] SET QUERY_STORE = ON
GO
ALTER DATABASE [GEOMEDIA] SET QUERY_STORE (OPERATION_MODE = READ_WRITE, CLEANUP_POLICY = (STALE_QUERY_THRESHOLD_DAYS = 30), DATA_FLUSH_INTERVAL_SECONDS = 900, INTERVAL_LENGTH_MINUTES = 60, MAX_STORAGE_SIZE_MB = 1000, QUERY_CAPTURE_MODE = AUTO, SIZE_BASED_CLEANUP_MODE = AUTO, MAX_PLANS_PER_QUERY = 200, WAIT_STATS_CAPTURE_MODE = ON)
GO
USE [GEOMEDIA]
GO
/****** Object:  Table [dbo].[COLLECTIONS]    Script Date: 06/05/2026 09:41:47 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE TABLE [dbo].[COLLECTIONS](
	[ID] [int] IDENTITY(1,1) NOT NULL,
	[TITLE] [varchar](50) NULL,
	[OWNERID] [int] NULL,
	[DESCRIPTION] [varchar](256) NULL,
	[DC] [datetime] NULL,
	[DM] [datetime] NULL,
	[EXCL_DATE_START] [datetime] NULL,
	[EXCL_DATE_END] [datetime] NULL,
	[RECURRENT] [int] NULL,
	[REMOTE_POSTING] [bit] NULL,
	[SEQUENTIALS] [nvarchar](max) NULL,
	[ICON] [varchar](20) NULL,
	[COLOR] [varchar](20) NULL,
	[CREATORS] [nvarchar](max) NULL,
	[VIEWERS] [nvarchar](max) NULL,
	[HASHTAGS] [nvarchar](max) NULL,
PRIMARY KEY CLUSTERED 
(
	[ID] ASC
)WITH (PAD_INDEX = OFF, STATISTICS_NORECOMPUTE = OFF, IGNORE_DUP_KEY = OFF, ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON, OPTIMIZE_FOR_SEQUENTIAL_KEY = OFF) ON [PRIMARY]
) ON [PRIMARY] TEXTIMAGE_ON [PRIMARY]
GO
/****** Object:  Table [dbo].[HASHTAGS]    Script Date: 06/05/2026 09:41:47 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE TABLE [dbo].[HASHTAGS](
	[ID] [int] IDENTITY(1,1) NOT NULL,
	[TITLE] [varchar](50) NULL,
	[DC] [datetime] NULL,
	[UC] [int] NULL,
PRIMARY KEY CLUSTERED 
(
	[ID] ASC
)WITH (PAD_INDEX = OFF, STATISTICS_NORECOMPUTE = OFF, IGNORE_DUP_KEY = OFF, ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON, OPTIMIZE_FOR_SEQUENTIAL_KEY = OFF) ON [PRIMARY],
UNIQUE NONCLUSTERED 
(
	[TITLE] ASC
)WITH (PAD_INDEX = OFF, STATISTICS_NORECOMPUTE = OFF, IGNORE_DUP_KEY = OFF, ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON, OPTIMIZE_FOR_SEQUENTIAL_KEY = OFF) ON [PRIMARY]
) ON [PRIMARY]
GO
/****** Object:  Table [dbo].[HYPERMEDIA]    Script Date: 06/05/2026 09:41:47 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE TABLE [dbo].[HYPERMEDIA](
	[ID] [int] IDENTITY(1,1) NOT NULL,
	[POST_ID] [int] NULL,
	[FILENAME] [varchar](512) NULL,
	[FILEPATH] [varchar](512) NULL,
	[MIME_TYPE] [varchar](32) NULL
) ON [PRIMARY]
GO
/****** Object:  Table [dbo].[INTERACTIONS]    Script Date: 06/05/2026 09:41:47 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE TABLE [dbo].[INTERACTIONS](
	[ID] [int] IDENTITY(1,1) NOT NULL,
	[POST_ID] [int] NULL,
	[OWNERID] [int] NULL,
	[DC] [datetime] NULL,
	[DM] [datetime] NULL,
	[SCOPE] [varchar](20) NULL,
PRIMARY KEY CLUSTERED 
(
	[ID] ASC
)WITH (PAD_INDEX = OFF, STATISTICS_NORECOMPUTE = OFF, IGNORE_DUP_KEY = OFF, ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON, OPTIMIZE_FOR_SEQUENTIAL_KEY = OFF) ON [PRIMARY]
) ON [PRIMARY]
GO
/****** Object:  Table [dbo].[INTERESTS]    Script Date: 06/05/2026 09:41:47 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE TABLE [dbo].[INTERESTS](
	[ID] [int] IDENTITY(1,1) NOT NULL,
	[USER_ID] [int] NULL,
	[EXTERNAL_REF] [int] NULL,
	[ENTITY] [varchar](200) NULL,
PRIMARY KEY CLUSTERED 
(
	[ID] ASC
)WITH (PAD_INDEX = OFF, STATISTICS_NORECOMPUTE = OFF, IGNORE_DUP_KEY = OFF, ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON, OPTIMIZE_FOR_SEQUENTIAL_KEY = OFF) ON [PRIMARY]
) ON [PRIMARY]
GO
/****** Object:  Table [dbo].[LOGS]    Script Date: 06/05/2026 09:41:47 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE TABLE [dbo].[LOGS](
	[ID] [int] IDENTITY(1,1) NOT NULL,
	[MESSAGE] [nvarchar](1000) NULL,
	[SCOPE] [varchar](30) NULL,
	[DATELOG] [datetime] NULL,
PRIMARY KEY CLUSTERED 
(
	[ID] ASC
)WITH (PAD_INDEX = OFF, STATISTICS_NORECOMPUTE = OFF, IGNORE_DUP_KEY = OFF, ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON, OPTIMIZE_FOR_SEQUENTIAL_KEY = OFF) ON [PRIMARY]
) ON [PRIMARY]
GO
/****** Object:  Table [dbo].[POSTS]    Script Date: 06/05/2026 09:41:47 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE TABLE [dbo].[POSTS](
	[ID] [int] IDENTITY(1,1) NOT NULL,
	[AUTHOR_ID] [int] NULL,
	[TITLE] [varchar](50) NULL,
	[COMMENT] [varchar](max) NULL,
	[DC] [datetime] NULL,
	[DM] [datetime] NULL,
	[LATITUDE] [float] NULL,
	[LONGITUDE] [float] NULL,
	[VISIBILITY_AREA_KM] [int] NULL,
	[EXCL_DATE_START] [datetime] NULL,
	[EXCL_DATE_END] [datetime] NULL,
	[RECURRENT] [int] NULL,
	[COLLECTION_ID] [int] NULL,
	[VIEWERS] [nvarchar](max) NULL,
	[ALTITUDE] [float] NULL,
PRIMARY KEY CLUSTERED 
(
	[ID] ASC
)WITH (PAD_INDEX = OFF, STATISTICS_NORECOMPUTE = OFF, IGNORE_DUP_KEY = OFF, ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON, OPTIMIZE_FOR_SEQUENTIAL_KEY = OFF) ON [PRIMARY]
) ON [PRIMARY] TEXTIMAGE_ON [PRIMARY]
GO
/****** Object:  Table [dbo].[REPORTS]    Script Date: 06/05/2026 09:41:47 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE TABLE [dbo].[REPORTS](
	[ID] [int] IDENTITY(1,1) NOT NULL,
	[USERID] [int] NULL,
	[MOTIVE] [varchar](2480) NULL,
	[POSTID] [int] NULL,
	[DC] [datetime] NULL,
	[KIND] [varchar](40) NULL,
	[PROCESSED] [bit] NULL,
PRIMARY KEY CLUSTERED 
(
	[ID] ASC
)WITH (PAD_INDEX = OFF, STATISTICS_NORECOMPUTE = OFF, IGNORE_DUP_KEY = OFF, ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON, OPTIMIZE_FOR_SEQUENTIAL_KEY = OFF) ON [PRIMARY]
) ON [PRIMARY]
GO
/****** Object:  Table [dbo].[USERS]    Script Date: 06/05/2026 09:41:47 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE TABLE [dbo].[USERS](
	[ID] [int] IDENTITY(1,1) NOT NULL,
	[USERNAME] [varchar](50) NULL,
	[PASSWORD] [nvarchar](256) NOT NULL,
	[EMAIL] [varchar](128) NULL,
	[NAME] [varchar](30) NULL,
	[SURNAME] [varchar](30) NULL,
	[DC] [datetime] NULL,
	[DM] [datetime] NULL,
	[PROFILE_PICTURE] [nvarchar](max) NULL,
	[LASTIP] [nvarchar](max) NULL,
	[OTPCODE] [varchar](6) NULL,
	[EMAIL_VERIFIED] [bit] NULL,
	[LASTHEADERS] [nvarchar](max) NULL,
	[OTPDATE] [datetime] NULL,
PRIMARY KEY CLUSTERED 
(
	[ID] ASC
)WITH (PAD_INDEX = OFF, STATISTICS_NORECOMPUTE = OFF, IGNORE_DUP_KEY = OFF, ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON, OPTIMIZE_FOR_SEQUENTIAL_KEY = OFF) ON [PRIMARY],
UNIQUE NONCLUSTERED 
(
	[USERNAME] ASC
)WITH (PAD_INDEX = OFF, STATISTICS_NORECOMPUTE = OFF, IGNORE_DUP_KEY = OFF, ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON, OPTIMIZE_FOR_SEQUENTIAL_KEY = OFF) ON [PRIMARY]
) ON [PRIMARY] TEXTIMAGE_ON [PRIMARY]
GO
ALTER TABLE [dbo].[COLLECTIONS] ADD  DEFAULT (getdate()) FOR [DM]
GO
ALTER TABLE [dbo].[INTERACTIONS] ADD  DEFAULT (getdate()) FOR [DM]
GO
ALTER TABLE [dbo].[LOGS] ADD  DEFAULT (getdate()) FOR [DATELOG]
GO
ALTER TABLE [dbo].[POSTS] ADD  DEFAULT (getdate()) FOR [DM]
GO
ALTER TABLE [dbo].[USERS] ADD  DEFAULT (getdate()) FOR [DM]
GO
ALTER TABLE [dbo].[USERS] ADD  DEFAULT (NULL) FOR [PROFILE_PICTURE]
GO
ALTER TABLE [dbo].[USERS] ADD  DEFAULT ((0)) FOR [EMAIL_VERIFIED]
GO
ALTER TABLE [dbo].[COLLECTIONS]  WITH CHECK ADD FOREIGN KEY([OWNERID])
REFERENCES [dbo].[USERS] ([ID])
GO
ALTER TABLE [dbo].[HYPERMEDIA]  WITH CHECK ADD FOREIGN KEY([POST_ID])
REFERENCES [dbo].[POSTS] ([ID])
GO
ALTER TABLE [dbo].[HYPERMEDIA]  WITH CHECK ADD FOREIGN KEY([POST_ID])
REFERENCES [dbo].[POSTS] ([ID])
GO
ALTER TABLE [dbo].[INTERACTIONS]  WITH CHECK ADD FOREIGN KEY([OWNERID])
REFERENCES [dbo].[USERS] ([ID])
GO
ALTER TABLE [dbo].[INTERACTIONS]  WITH CHECK ADD FOREIGN KEY([OWNERID])
REFERENCES [dbo].[USERS] ([ID])
GO
ALTER TABLE [dbo].[INTERACTIONS]  WITH CHECK ADD FOREIGN KEY([POST_ID])
REFERENCES [dbo].[POSTS] ([ID])
GO
ALTER TABLE [dbo].[INTERACTIONS]  WITH CHECK ADD FOREIGN KEY([POST_ID])
REFERENCES [dbo].[POSTS] ([ID])
GO
ALTER TABLE [dbo].[INTERESTS]  WITH CHECK ADD FOREIGN KEY([USER_ID])
REFERENCES [dbo].[USERS] ([ID])
GO
ALTER TABLE [dbo].[INTERESTS]  WITH CHECK ADD FOREIGN KEY([USER_ID])
REFERENCES [dbo].[USERS] ([ID])
GO
ALTER TABLE [dbo].[POSTS]  WITH CHECK ADD FOREIGN KEY([AUTHOR_ID])
REFERENCES [dbo].[USERS] ([ID])
GO
ALTER TABLE [dbo].[POSTS]  WITH CHECK ADD FOREIGN KEY([AUTHOR_ID])
REFERENCES [dbo].[USERS] ([ID])
GO
ALTER TABLE [dbo].[POSTS]  WITH CHECK ADD FOREIGN KEY([COLLECTION_ID])
REFERENCES [dbo].[COLLECTIONS] ([ID])
GO
ALTER TABLE [dbo].[REPORTS]  WITH CHECK ADD FOREIGN KEY([POSTID])
REFERENCES [dbo].[POSTS] ([ID])
GO
ALTER TABLE [dbo].[REPORTS]  WITH CHECK ADD FOREIGN KEY([POSTID])
REFERENCES [dbo].[POSTS] ([ID])
GO
ALTER TABLE [dbo].[REPORTS]  WITH CHECK ADD FOREIGN KEY([USERID])
REFERENCES [dbo].[USERS] ([ID])
GO
ALTER TABLE [dbo].[REPORTS]  WITH CHECK ADD FOREIGN KEY([USERID])
REFERENCES [dbo].[USERS] ([ID])
GO
/****** Object:  StoredProcedure [dbo].[AUTH_CHECK_OTP]    Script Date: 06/05/2026 09:41:47 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE  PROCEDURE [dbo].[AUTH_CHECK_OTP]
@JSON NVARCHAR(MAX)
AS
BEGIN
    DECLARE @MSG NVARCHAR(1000) 

    IF EXISTS(
        SELECT * FROM USERS WHERE USERNAME=JSON_VALUE(@JSON,'$.USERNAME') AND OTPCODE = CAST(JSON_VALUE(@JSON,'$.OTP') AS INT)
        AND DATEDIFF(hour,OTPDATE,GETDATE())<=1 --OTP generated within an hour
    )
    BEGIN
        UPDATE USERS SET EMAIL_VERIFIED = 1, DM=GETDATE(), OTPDATE=GETDATE() WHERE USERNAME= JSON_VALUE(@JSON,'$.USERNAME')
        SELECT 1 AS AUTH, ID AS UID, * FROM USERS WHERE USERNAME= JSON_VALUE(@JSON,'$.USERNAME')
    END
    ELSE
    BEGIN
        SET @MSG = CONCAT('Wrong OPT for users:',JSON_VALUE(@JSON,'$.USERNAME'),' CODE PROVIDED: ' , JSON_VALUE(@JSON,'$.OTP'))
        EXEC LOG_WRITE @MESSAGE=@MSG, @SCOPE='OTP'
        SELECT 0 AS AUTH
    END
END





GO
/****** Object:  StoredProcedure [dbo].[AUTH_CHECK_USERNAME]    Script Date: 06/05/2026 09:41:47 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[AUTH_CHECK_USERNAME]
@JSON NVARCHAR(MAX)
AS
BEGIN
    IF EXISTS(SELECT * FROM USERS WHERE USERNAME=JSON_VALUE(@JSON,'$.USERNAME'))
        SELECT 0 AS OK, 'USERNAME TAKEN' AS MSG
    ELSE 
        SELECT 1 AS OK, 'USERNAME AVAILABLE' AS MSG
END

GO
/****** Object:  StoredProcedure [dbo].[AUTH_LOGIN]    Script Date: 06/05/2026 09:41:47 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

CREATE PROCEDURE [dbo].[AUTH_LOGIN]
@JSON NVARCHAR(MAX)
AS
BEGIN

    DECLARE @USERNAME VARCHAR(50) = NULL,@EMAIL VARCHAR(128) = NULL, @PASSWORD NVARCHAR(256), @IP VARCHAR(200), @HEADERS NVARCHAR(MAX)

    SELECT  @USERNAME=EMAIL, @EMAIL=EMAIL,@PASSWORD=PASSWORD, @IP=IP, @HEADERS=HEADERS FROM OPENJSON(@JSON,'$')WITH(
        EMAIL VARCHAR(128),
        PASSWORD NVARCHAR(256),
         IP VARCHAR(200),
        HEADERS NVARCHAR(MAX)
    )

    IF EXISTS (
        SELECT * FROM DBO.USERS WHERE (USERNAME=@USERNAME OR EMAIL=@EMAIL) AND [PASSWORD] = @PASSWORD
    )
    BEGIN
        SELECT 1 AS AUTH, ID AS UID, USERNAME, EMAIL, NAME, SURNAME FROM USERS 
        WHERE (USERNAME=@USERNAME OR EMAIL=@EMAIL) 

        UPDATE USERS SET LASTIP=@IP, LASTHEADERS=@HEADERS WHERE (USERNAME=@USERNAME OR EMAIL=@EMAIL) 
    END
    ELSE
    BEGIN
        SELECT 0 AS AUTH, 'Invalid cretential' as MSG
    
    END



END



GO
/****** Object:  StoredProcedure [dbo].[AUTH_PSW_FORGOTTEN]    Script Date: 06/05/2026 09:41:47 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[AUTH_PSW_FORGOTTEN]
@JSON VARCHAR(MAX)
AS
BEGIN
	DECLARE @OTP VARCHAR(6) = (SELECT RIGHT('000000' + CAST(CAST(RAND() * 1000000 AS INT) AS VARCHAR(6)), 6))

	UPDATE U SET OTPCODE =@OTP, OTPDATE=GETDATE()
	FROM USERS U
	INNER JOIN OPENJSON(@JSON,'$')WITH(
		USERNAMEMAIL VARCHAR(128)
	) J ON J.USERNAMEMAIL = U.EMAIL OR J.USERNAMEMAIL = U.USERNAME

	SELECT @OTP AS OTP, 2 as AUTH, U.EMAIL, U.USERNAME  --IN ORDER TO SEND THE OTP CODE VIA EMAIL
	FROM  USERS U
	INNER JOIN OPENJSON(@JSON,'$')WITH(
		USERNAMEMAIL VARCHAR(128)
	) J ON J.USERNAMEMAIL = U.EMAIL OR J.USERNAMEMAIL = U.USERNAME

END



GO
/****** Object:  StoredProcedure [dbo].[AUTH_PSW_RESET]    Script Date: 06/05/2026 09:41:47 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[AUTH_PSW_RESET]
@JSON NVARCHAR(MAX)
AS
BEGIN
	
	DECLARE @MSG VARCHAR(1000)

	UPDATE U
	SET PASSWORD=J.NEWPASSWORD,
	DM=GETDATE(),
	LASTHEADERS= HEADERS,
	LASTIP = IP
	FROM USERS U
	INNER JOIN OPENJSON(@JSON,'$')WITH(
			USERNAMEMAIL VARCHAR(128),
			NEWPASSWORD NVARCHAR(512),
			OTP VARCHAR(6),
			IP VARCHAR(200),
			HEADERS NVARCHAR(MAX) AS JSON
	) J ON (J.USERNAMEMAIL = U.EMAIL OR J.USERNAMEMAIL = U.USERNAME)  AND J.OTP = U.OTPCODE
	
	IF @@ROWCOUNT>0
	BEGIN
		SELECT 1 AS OK
		SET @MSG = CONCAT('PASSWORD RESET COMPLETED USER: ', JSON_VALUE(@JSON,'$.USERNAME'), ' OTP: ', JSON_VALUE(@JSON,'$.OTP'))
		EXEC LOG_WRITE @MESSAGE=@MSG, @SCOPE='PSWRESET'
	END
	ELSE
	BEGIN
		SELECT 0 AS OK
		SET @MSG = CONCAT('wrong otp on psw reset, USER: ', JSON_VALUE(@JSON,'$.USERNAMEMAIL'), ' OTP: ', JSON_VALUE(@JSON,'$.OTP'))
		EXEC LOG_WRITE @MESSAGE=@MSG, @SCOPE='PSWRESET'
	END
END

GO
/****** Object:  StoredProcedure [dbo].[AUTH_SIGNIN]    Script Date: 06/05/2026 09:41:47 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[AUTH_SIGNIN]
@JSON NVARCHAR(MAX)
AS
BEGIN
    
    DECLARE @USERNAME VARCHAR(50), @EMAIL VARCHAR(128),@PASSWORD NVARCHAR(256),@NAME VARCHAR(30)=NULL, @SURNAME VARCHAR(30)=NULL, @IP VARCHAR(200), @HEADERS NVARCHAR(MAX)

    SELECT  @USERNAME=USERNAME, @EMAIL=EMAIL,@PASSWORD=PASSWORD, @NAME=NAME,@SURNAME=SURNAME, @HEADERS=HEADERS, @IP=IP FROM OPENJSON(@JSON,'$')WITH(
        USERNAME VARCHAR(50),
        EMAIL VARCHAR(128),
        PASSWORD NVARCHAR(256),
        NAME VARCHAR(30),
        SURNAME VARCHAR(30),
        IP VARCHAR(200),
		HEADERS NVARCHAR(MAX) AS JSON
    )

    IF NOT EXISTS(
        SELECT * FROM USERS WHERE ([EMAIL]=@EMAIL OR USERNAME = @USERNAME) AND EMAIL_VERIFIED=0  --IF EMAIL NOT VERIFIED, THEN RESEND OTP VIA SIGNIN
    )
    BEGIN
    
        DECLARE @OTP VARCHAR(6) = (SELECT RIGHT('000000' + CAST(CAST(RAND() * 1000000 AS INT) AS VARCHAR(6)), 6))
        INSERT INTO USERS(EMAIL, USERNAME, NAME, SURNAME, DC, DM,[PASSWORD],OTPCODE, OTPDATE, LASTIP,LASTHEADERS)
        VALUES(@EMAIL,@USERNAME, @NAME, @SURNAME, GETDATE(),GETDATE(),@PASSWORD,@OTP, GETDATE(),@IP,@HEADERS);

        SELECT @OTP AS OTP, 2 AS AUTH, 'pending otp' as MSG
    END
    ELSE
    BEGIN
        SELECT 0 AS AUTH, 'Username or email already taken' as MSG
    END
END


GO
/****** Object:  StoredProcedure [dbo].[COLLECTION_MERGE]    Script Date: 06/05/2026 09:41:47 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

CREATE PROCEDURE [dbo].[COLLECTION_MERGE]
@JSON NVARCHAR(MAX)
AS
BEGIN
	BEGIN TRANSACTION T1
    BEGIN TRY
        DECLARE @Result TABLE (ID INT);

        MERGE INTO COLLECTIONS AS target
        USING (
            SELECT *
            FROM OPENJSON(@json)
            WITH (
                id INT,
                title VARCHAR(50),
                ownerid INT,
                description VARCHAR(256),
                excl_date_start DATETIME,
                excl_date_end DATETIME,
                recurrent INT,
                remote_posting BIT,
                sequentials NVARCHAR(MAX) AS JSON,
                icon VARCHAR(50),
                color VARCHAR(20),
                creators NVARCHAR(MAX) AS JSON,
                viewers NVARCHAR(MAX) AS JSON,
                hashtags NVARCHAR(MAX) AS JSON
            )
        ) AS source
        ON target.ID = source.ID

        WHEN MATCHED THEN
            UPDATE SET
                TITLE = source.TITLE,
                OWNERID = source.OWNERID,
                DESCRIPTION = source.DESCRIPTION,
                DM = GETDATE(),
                EXCL_DATE_START = source.EXCL_DATE_START,
                EXCL_DATE_END = source.EXCL_DATE_END,
                RECURRENT = source.RECURRENT,
                REMOTE_POSTING = source.REMOTE_POSTING,
                SEQUENTIALS = source.SEQUENTIALS,
                ICON = source.ICON,
                COLOR = source.COLOR,
                CREATORS=source.CREATORS,
                VIEWERS=source.VIEWERS,
                HASHTAGS = source.HASHTAGS


        WHEN NOT MATCHED THEN
            INSERT (
                TITLE, OWNERID, DESCRIPTION, DC,DM,
                CREATORS, VIEWERS, EXCL_DATE_START, EXCL_DATE_END,
                RECURRENT,  REMOTE_POSTING,
                SEQUENTIALS, ICON,COLOR,HASHTAGS
            )
            VALUES (
                source.TITLE, source.OWNERID, source.DESCRIPTION, GETDATE(),GETDATE(),
                source.CREATORS, source.VIEWERS, source.EXCL_DATE_START, source.EXCL_DATE_END,
                source.RECURRENT,source.REMOTE_POSTING,
                source.SEQUENTIALS,source.ICON,source.COLOR, source.HASHTAGS
            )
        OUTPUT inserted.ID INTO @Result;


        --- INSERT TAGS
        INSERT INTO HASHTAGS (TITLE,DC,UC)
        SELECT value,GETDATE(),JSON_VALUE(@JSON,'$.OWNERID') FROM OPENJSON(@JSON,'$.HASHTAGS')
        WHERE NOT EXISTS (SELECT * FROM HASHTAGS WHERE TITLE=value)
        
        COMMIT TRANSACTION T1;
        SELECT ID AS ID, 1 AS OK FROM @Result;

    END TRY
    BEGIN CATCH
        ROLLBACK TRANSACTION T1;
        SELECT -1 AS ID, 0 AS OK, ERROR_MESSAGE() AS MSG
    END CATCH

END

GO
/****** Object:  StoredProcedure [dbo].[COLLECTION_POSTS_GET]    Script Date: 06/05/2026 09:41:47 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

CREATE PROCEDURE [dbo].[COLLECTION_POSTS_GET]
@JSON NVARCHAR(MAX)
AS
BEGIN
    
        DECLARE @COLID INT = (SELECT JSON_VALUE(@JSON,'$.collectionid'))
        ;;
        WITH SEQ AS (

            SELECT order_id,post_id
            FROM OPENJSON((SELECT SEQUENTIALS FROM COLLECTIONS WHERE ID = @COLID),'$') WITH(
                order_id int,
                post_id int
            ) 
        )
        SELECT P.*, SEQ.order_id, SEQ.post_id FROM POSTS P
        LEFT JOIN SEQ ON SEQ.post_id = P.ID
        WHERE P.COLLECTION_ID = @COLID
        ORDER BY SEQ.order_id
END


GO
/****** Object:  StoredProcedure [dbo].[COLLECTIONS_BY_TAG]    Script Date: 06/05/2026 09:41:47 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO



CREATE PROCEDURE [dbo].[COLLECTIONS_BY_TAG]
@UID INT
AS
BEGIN

    WITH HASHTAG_INTEREST AS (
        SELECT HT.TITLE FROM INTERESTS I
        INNER JOIN HASHTAGS HT ON I.EXTERNAL_REF = HT.ID AND I.ENTITY='HASHTAG'
        WHERE I.[USER_ID]=@UID
    )
    SELECT HI.TITLE AS TAG,C.* FROM COLLECTIONS C
    INNER JOIN HASHTAG_INTEREST HI ON HI.TITLE IN 
    (
        SELECT VALUE FROM OPENJSON(C.HASHTAGS,'$')
    )
    WHERE 
    ( --EXCLUSIVITY
        GETDATE()> ISNULL(EXCL_DATE_START,DATEADD(DAY,-1,GETDATE())) AND GETDATE() < ISNULL(EXCL_DATE_END,DATEADD(DAY,1,GETDATE())) --IF NULL, MAKE IT FROM YESTERDAY TILL TOMORROW
        AND
        (
            (@UID IN (SELECT VALUE FROM OPENJSON(VIEWERS,'$')) OR VIEWERS IS NULL OR VIEWERS = '[]') --VIEWER
        OR
            (@UID IN (SELECT VALUE FROM OPENJSON(CREATORS,'$')) OR CREATORS IS NULL OR CREATORS = '[]') -- CREATOR CAN BE VIEWER NO NEED TO SPECIFY
        )
    )OR OWNERID=@UID


END

GO
/****** Object:  StoredProcedure [dbo].[COLLECTIONS_GET]    Script Date: 06/05/2026 09:41:47 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO


CREATE PROCEDURE [dbo].[COLLECTIONS_GET]
@JSON NVARCHAR(MAX)
AS
BEGIN


	DECLARE @UID INT, @MODE VARCHAR(10) -- R (reading/viewing post), W (writing) to check if creator

	SELECT @UID=uid, @MODE=mode FROM
	OPENJSON(@JSON,'$')WITH(
		uid INT,
		mode VARCHAR(10)
	)

	IF(@MODE='W') --- just collection not expired, and where user is creator || is owner of the collection
	BEGIN

		SELECT * FROM COLLECTIONS
		WHERE 
		( --EXCLUSIVITY
			(
				(RECURRENT IS NULL  and (GETDATE()> ISNULL(EXCL_DATE_START,DATEADD(DAY,-1,GETDATE())) OR GETDATE() < ISNULL(EXCL_DATE_END,DATEADD(DAY,1,GETDATE()))) --IF NULL, MAKE IT FROM YESTERDAY TILL TOMORROW
				) -- NO RECURRENTCY
				OR 
				(RECURRENT =1 AND (DAY(GETDATE()) BETWEEN DAY(EXCL_DATE_START) AND DAY(EXCL_DATE_START))) --EACH DAYS, IGNORING MONTHS
				OR
				(RECURRENT = 2 AND 
					(DATEFROMPARTS(2000, MONTH(GETDATE()), DAY(GETDATE()))
					BETWEEN DATEFROMPARTS(2000, MONTH(EXCL_DATE_START), DAY(EXCL_DATE_START))
					  AND DATEFROMPARTS(2000, MONTH(EXCL_DATE_END), DAY(EXCL_DATE_END)))
				) -- EVERY YEAR, SAME DAYS SAME MONTH
			)
			AND (@UID IN (SELECT VALUE FROM OPENJSON(CREATORS,'$')) OR CREATORS IS NULL OR CREATORS ='[]') -- is creator or hasn't exclusivity
		)OR OWNERID=@UID
	END
	ELSE IF(@MODE='R') --ALL AS LIST
	BEGIN

		SELECT * FROM COLLECTIONS
		WHERE 
		( --EXCLUSIVITY
			(
				(@UID IN (SELECT VALUE FROM OPENJSON(VIEWERS,'$')) OR VIEWERS IS NULL OR VIEWERS = '[]') --VIEWER
			OR
				(@UID IN (SELECT VALUE FROM OPENJSON(CREATORS,'$')) OR CREATORS IS NULL OR CREATORS = '[]') -- CREATOR CAN BE VIEWER NO NEED TO SPECIFY
			)
			AND
			(
				(RECURRENT IS NULL  and (GETDATE()> ISNULL(EXCL_DATE_START,DATEADD(DAY,-1,GETDATE())) OR GETDATE() < ISNULL(EXCL_DATE_END,DATEADD(DAY,1,GETDATE()))) --IF NULL, MAKE IT FROM YESTERDAY TILL TOMORROW
				) -- NO RECURRENTCY
				OR 
				(RECURRENT =1 AND (DAY(GETDATE()) BETWEEN DAY(EXCL_DATE_START) AND DAY(EXCL_DATE_START))) --EACH DAYS, IGNORING MONTHS
				OR
				(RECURRENT = 2 AND 
					(DATEFROMPARTS(2000, MONTH(GETDATE()), DAY(GETDATE()))
					BETWEEN DATEFROMPARTS(2000, MONTH(EXCL_DATE_START), DAY(EXCL_DATE_START))
					  AND DATEFROMPARTS(2000, MONTH(EXCL_DATE_END), DAY(EXCL_DATE_END)))
				) -- EVERY YEAR, SAME DAYS SAME MONTH
			)
		)OR OWNERID=@UID
	END
	ELSE IF(@MODE='TOP') -- TOP 20 MOST POPULAR
	BEGIN
		SELECT TOP 20 C.*,
			(SELECT COUNT(P.ID) FROM POSTS P WHERE COLLECTION_ID=C.ID) AS NR_POSTS
		FROM COLLECTIONS C
		WHERE 
		( --EXCLUSIVITY
			(
				(@UID IN (SELECT VALUE FROM OPENJSON(VIEWERS,'$')) OR VIEWERS IS NULL OR VIEWERS = '[]') --VIEWER
			OR
				(@UID IN (SELECT VALUE FROM OPENJSON(CREATORS,'$')) OR CREATORS IS NULL OR CREATORS = '[]') -- CREATOR CAN BE VIEWER NO NEED TO SPECIFY
			)
			AND
			(
				(RECURRENT IS NULL  and (GETDATE()> ISNULL(EXCL_DATE_START,DATEADD(DAY,-1,GETDATE())) OR GETDATE() < ISNULL(EXCL_DATE_END,DATEADD(DAY,1,GETDATE()))) --IF NULL, MAKE IT FROM YESTERDAY TILL TOMORROW
				) -- NO RECURRENTCY
				OR 
				(RECURRENT =1 AND (DAY(GETDATE()) BETWEEN DAY(EXCL_DATE_START) AND DAY(EXCL_DATE_START))) --EACH DAYS, IGNORING MONTHS
				OR
				(RECURRENT = 2 AND 
					(DATEFROMPARTS(2000, MONTH(GETDATE()), DAY(GETDATE()))
					BETWEEN DATEFROMPARTS(2000, MONTH(EXCL_DATE_START), DAY(EXCL_DATE_START))
					  AND DATEFROMPARTS(2000, MONTH(EXCL_DATE_END), DAY(EXCL_DATE_END)))
				) -- EVERY YEAR, SAME DAYS SAME MONTH
			)
		)OR OWNERID=@UID
		ORDER BY (SELECT COUNT(P.ID) FROM POSTS P WHERE COLLECTION_ID=C.ID) DESC
	END
	ELSE IF(@MODE='HOT') -- TRENDING (LAST 4H)
	BEGIN
		SELECT TOP 20 C.*,
			(SELECT COUNT(P.ID) FROM POSTS P WHERE COLLECTION_ID=C.ID ) AS NR_POSTS_ALLTIME,
			(SELECT COUNT(P.ID) FROM POSTS P WHERE COLLECTION_ID=C.ID AND P.DC >= DATEADD(HOUR, -4, GETDATE()) ) AS NR_POSTS_HOT
		FROM COLLECTIONS C
		WHERE 
		( --EXCLUSIVITY
			(
				(@UID IN (SELECT VALUE FROM OPENJSON(VIEWERS,'$')) OR VIEWERS IS NULL OR VIEWERS = '[]') --VIEWER
			OR
				(@UID IN (SELECT VALUE FROM OPENJSON(CREATORS,'$')) OR CREATORS IS NULL OR CREATORS = '[]') -- CREATOR CAN BE VIEWER NO NEED TO SPECIFY
			)
			AND
			(
				(RECURRENT IS NULL  and (GETDATE()> ISNULL(EXCL_DATE_START,DATEADD(DAY,-1,GETDATE())) OR GETDATE() < ISNULL(EXCL_DATE_END,DATEADD(DAY,1,GETDATE()))) --IF NULL, MAKE IT FROM YESTERDAY TILL TOMORROW
				) -- NO RECURRENTCY
				OR 
				(RECURRENT =1 AND (DAY(GETDATE()) BETWEEN DAY(EXCL_DATE_START) AND DAY(EXCL_DATE_START))) --EACH DAYS, IGNORING MONTHS
				OR
				(RECURRENT = 2 AND 
					(DATEFROMPARTS(2000, MONTH(GETDATE()), DAY(GETDATE()))
					BETWEEN DATEFROMPARTS(2000, MONTH(EXCL_DATE_START), DAY(EXCL_DATE_START))
					  AND DATEFROMPARTS(2000, MONTH(EXCL_DATE_END), DAY(EXCL_DATE_END)))
				) -- EVERY YEAR, SAME DAYS SAME MONTH
			)
		)OR OWNERID=@UID
		ORDER BY 
		(SELECT COUNT(P.ID) FROM POSTS P WHERE COLLECTION_ID=C.ID AND P.DC >= DATEADD(HOUR, -4, GETDATE()) )
		,(SELECT COUNT(P.ID) FROM POSTS P WHERE COLLECTION_ID=C.ID) DESC
	END

END

GO

/****** Object:  StoredProcedure [dbo].[COLLECTIONS_GET_FULLCOLLECTION]    Script Date: 06/05/2026 09:41:47 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO


CREATE PROCEDURE [dbo].[COLLECTIONS_GET_FULLCOLLECTION]
@JSON NVARCHAR(MAX)
AS
BEGIN

	SELECT C.* 
	FROM COLLECTIONS C
	INNER JOIN	OPENJSON(@JSON,'$') WITH(collectionid INT, uid INT) JJ ON JJ.collectionid = C.ID 
	-- AND JJ.uid IN (
	-- 	SELECT VALUE FROM OPENJSON(C.VIEWERS,'$')
	-- ) OR JJ.uid IN (
	-- 	SELECT VALUE FROM OPENJSON(C.CREATORS,'$')
	-- ) OR JJ.uid = C.OWNERID

END

GO
/****** Object:  StoredProcedure [dbo].[HPMEDIA_GETFILES]    Script Date: 06/05/2026 09:41:47 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[HPMEDIA_GETFILES]
@POSTID INT
AS
BEGIN
    SELECT * FROM HYPERMEDIA WHERE POST_ID=@POSTID
END

GO
/****** Object:  StoredProcedure [dbo].[HPMEDIA_MERGEFILE]    Script Date: 06/05/2026 09:41:47 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

  CREATE procedure [dbo].[HPMEDIA_MERGEFILE]
  @FILESNAME_ATTACHED NVARCHAR(MAX), @POSTID INT
  AS
  BEGIN
    BEGIN TRY

        MERGE HYPERMEDIA AS target
            USING (
                SELECT *
                FROM OPENJSON(@FILESNAME_ATTACHED)
                WITH (
                    filepath NVARCHAR(500),
                    filename NVARCHAR(255),
                    mimetype VARCHAR(32),
                    post_id INT
                ) WHERE filename IS NOT NULL
            ) AS source
            ON  target.FILENAME = source.filename
            AND target.POST_ID = source.post_id

            WHEN NOT MATCHED BY TARGET THEN
                INSERT (FILEPATH, FILENAME,  POST_ID, MIME_TYPE)
                VALUES (source.filepath, source.filename,  source.post_id,source.mimetype)

            WHEN NOT MATCHED BY SOURCE
            AND target.POST_ID IN (
                SELECT POST_ID FROM OPENJSON(@FILESNAME_ATTACHED) WITH (POST_ID INT)
            )
            THEN DELETE;

        SELECT 1 AS OK
    END TRY
    BEGIN CATCH
        SELECT 0 AS OK, ERROR_MESSAGE() AS MSG
    END CATCH

  END

GO
/****** Object:  StoredProcedure [dbo].[HPMEDIA_REMOVE]    Script Date: 06/05/2026 09:41:47 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[HPMEDIA_REMOVE]
@JSON NVARCHAR(MAX)
AS
BEGIN
    DECLARE @POSTID INT, @FILENAME VARCHAR(256)
    SELECT @POSTID=postid, @FILENAME=filename
    FROM OPENJSON(@JSON,'$')WITH(
        postid INT,
        filename VARCHAR(256)
    )

    DELETE FROM HYPERMEDIA WHERE POST_ID=@POSTID
END

GO
/****** Object:  StoredProcedure [dbo].[INTERACTIONS_LIKEPOST]    Script Date: 06/05/2026 09:41:47 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[INTERACTIONS_LIKEPOST]
@JSON NVARCHAR(MAX)
AS
BEGIN

    
    DECLARE @POSTID INT, @UID INT 
    SELECT @POSTID=postid,@UID=uid
    FROM OPENJSON(@json,'$')
    WITH (
        postid INT ,
        uid INT 
    )
    
    DECLARE @OPERATION VARCHAR(1)

    IF EXISTS(SELECT * FROM INTERACTIONS WHERE POST_ID=@POSTID AND OWNERID=@UID AND SCOPE='LIKE') 
    BEGIN
        DELETE FROM INTERACTIONS WHERE POST_ID=@POSTID AND OWNERID=@UID AND SCOPE='LIKE'
        SET @OPERATION='D'
    END
    ELSE
    BEGIN
        INSERT INTO INTERACTIONS (POST_ID,OWNERID,DC,DM,SCOPE)
        SELECT @POSTID, @UID, GETDATE(),GETDATE(),'LIKE'

        SET @OPERATION='I'
    END

    select 1 as OK, @OPERATION as OPERATION

END



GO
/****** Object:  StoredProcedure [dbo].[LOG_WRITE]    Script Date: 06/05/2026 09:41:47 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

CREATE PROCEDURE [dbo].[LOG_WRITE]
@MESSAGE NVARCHAR(1000), @SCOPE VARCHAR(30) ='DEBUG'
AS
BEGIN
	INSERT INTO LOGS(MESSAGE,SCOPE)
	VALUES (@MESSAGE, @SCOPE)

END

GO
/****** Object:  StoredProcedure [dbo].[POST_BY_AUTHOR]    Script Date: 06/05/2026 09:41:47 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO



CREATE PROCEDURE [dbo].[POST_BY_AUTHOR]
@JSON as NVARCHAR(MAX)
AS
BEGIN

    DECLARE @UID INT, @AUTHOR_ID INT;
    SELECT @UID = uid, @AUTHOR_ID= authorid
    FROM OPENJSON(@JSON,'$')WITH(
        uid INT,
        authorid INT
    )

    IF(@AUTHOR_ID=@UID) --SELF POST
    BEGIN
        SELECT DISTINCT P.*, C.ICON,C.COLOR FROM POSTS  P 
        INNER JOIN COLLECTIONS C ON P.COLLECTION_ID = C.ID
        WHERE AUTHOR_ID=@AUTHOR_ID
    END


END


GO
/****** Object:  StoredProcedure [dbo].[POST_DELETE]    Script Date: 06/05/2026 09:41:47 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO


CREATE PROCEDURE [dbo].[POST_DELETE]
    @POSTID INT,
    @PASSWORD VARCHAR(128)
AS
BEGIN
    DECLARE @MSG NVARCHAR(1000)

    IF EXISTS ( 
        SELECT * FROM USERS WHERE 
        ID = (SELECT AUTHOR_ID FROM POSTS WHERE ID = @POSTID) 
        AND PASSWORD = @PASSWORD
    )
    BEGIN
        DELETE FROM INTERACTIONS WHERE POST_ID = @POSTID;
        DELETE FROM HYPERMEDIA WHERE POST_ID = @POSTID;
        DELETE FROM POSTS WHERE ID = @POSTID;
        DELETE FROM REPORTS WHERE POSTID=@POSTID
        SELECT 1 AS OK
        SET @MSG =  CONCAT('DELETED POST', CAST(@POSTID AS VARCHAR(200)))
    END
    ELSE
    BEGIN
        SELECT 0 as OK
        SET @MSG =  CONCAT('CANNOT DELETE POST, WRONG PSW OR POSTID: ', CAST(@POSTID AS VARCHAR(200)), @PASSWORD)
    END

    EXEC dbo.LOG_WRITE @MESSAGE = @MSG, @SCOPE='POST_DELETE'


END





GO
/****** Object:  StoredProcedure [dbo].[POST_GET_COLLECTION]    Script Date: 06/05/2026 09:41:47 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[POST_GET_COLLECTION]
@COLLECTION_ID INT
AS
BEGIN
	SELECT * FROM POSTS
	--WHERE COLLECTION_ID =@COLLECTION_ID
END

GO
/****** Object:  StoredProcedure [dbo].[POST_GET_FULLPOST]    Script Date: 06/05/2026 09:41:47 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

CREATE PROCEDURE [dbo].[POST_GET_FULLPOST]
@JSON NVARCHAR(MAX)
AS
BEGIN

	--- COUNTING THE VIEW
	INSERT INTO INTERACTIONS (POST_ID,OWNERID,DC,DM,SCOPE)
	SELECT postid, uid, GETDATE(), GETDATE(), 'VIEW'
	FROM OPENJSON(@JSON,'$')WITH(
			postid int,
			uid int
	) J
	WHERE NOT EXISTS ( SELECT * FROM INTERACTIONS WHERE POST_ID=J.postid and OWNERID=J.uid AND SCOPE='VIEW')

	--- RETURING DATA WITH STATS

	;;
	WITH POST_STAT AS (
		SELECT POST_ID,
		SUM(IIF(SCOPE='LIKE',1,0)) AS NUM_LIKES,
		SUM(IIF(SCOPE='VIEW',1,0)) AS NUM_VIEWS
		FROM INTERACTIONS 
		WHERE POST_ID= JSON_VALUE(@JSON,'$.postid')
		GROUP BY POST_ID
	)
	SELECT DISTINCT P.*,
		(SELECT COUNT(*) FROM INTERACTIONS WHERE SCOPE='LIKE' AND POST_ID=P.ID AND OWNERID=JSON_VALUE(@JSON,'$.uid')) AS LIKED_BY_CURR_USER,
		PS.NUM_LIKES,
		PS.NUM_VIEWS,
		C.COLOR,
		C.ICON,
		C.TITLE AS COLLECTION_NAME,
		C.REMOTE_POSTING AS REMOTE_POSTING_ENABLED
	FROM POSTS P
	INNER JOIN COLLECTIONS C ON C.ID = P.COLLECTION_ID
	LEFT JOIN INTERACTIONS I ON P.ID = I.POST_ID AND I.SCOPE='VIEW'
	LEFT JOIN POST_STAT PS ON PS.POST_ID = P.ID
	WHERE P.ID=JSON_VALUE(@JSON,'$.postid')
	-- GROUP BY P.ID, P.AUTHOR_ID,P.COMMENT,P.DC,P.DM,P.LATITUDE, P.TITLE,P.LONGITUDE,P.VISIBILITY_AREA_KM,P.EXCL_DATE_START,P.EXCL_DATE_END,P.VIEWERS
	

END


GO
/****** Object:  StoredProcedure [dbo].[POST_GET_MAP]    Script Date: 06/05/2026 09:41:47 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE  PROCEDURE [dbo].[POST_GET_MAP]
@UID INT, @COLLECTIONS_CHOSEN NVARCHAR(MAX)=NULL
AS
BEGIN




    ---------------------------------------------------------
    --- formatting for sequentiality (order of the posts)

    -- -- DECLARE  @uid INT=1 , @COLLECTIONS_CHOSEN JSON='[35,36,37,38,39]'

    DECLARE @SEQQ TABLE (order_id  int, post_id int)
    ;;
    WITH SEQ AS(
        SELECT j.order_id, j.post_id
        FROM COLLECTIONS c
        CROSS APPLY OPENJSON(c.SEQUENTIALS, '$')
        WITH (
            order_id INT,
            post_id INT
        ) j
        WHERE c.ID IN (SELECT VALUE FROM OPENJSON(@COLLECTIONS_CHOSEN, '$'))
    )
    INSERT INTO @SEQQ(order_id,post_id)
    SELECT order_id,post_id FROM SEQ;

    ---------------------------------------------------------

    --- SEQUENTIALITY POSTS
    ; WITH NEXTPOST AS( --- FIRST POST NOT VIEWED
        SELECT TOP 1 P.*, SEQ.order_id, SEQ.post_id
        FROM POSTS P
        LEFT JOIN @SEQQ SEQ ON SEQ.post_id = P.ID
        WHERE P.COLLECTION_ID IN (SELECT VALUE FROM OPENJSON(@COLLECTIONS_CHOSEN,'$'))
        AND P.ID NOT IN (
            SELECT POST_ID FROM INTERACTIONS WHERE OWNERID=@UID AND SCOPE='VIEW'
        )
        ORDER BY SEQ.order_id ASC
    )
    SELECT
    P.ID,P.AUTHOR_ID,P.TITLE,P.COMMENT,P.DC,P.DM,P.LATITUDE,P.LONGITUDE,P.VISIBILITY_AREA_KM,P.EXCL_DATE_START,P.EXCL_DATE_END,P.RECURRENT,P.COLLECTION_ID,P.VIEWERS, SEQ.order_id, SEQ.post_id, C.COLOR, C.ICON,C.TITLE AS CATEGORY_NAME
    FROM POSTS P
    LEFT JOIN @SEQQ SEQ ON SEQ.post_id = P.ID
    INNER JOIN COLLECTIONS C ON C.ID = P.COLLECTION_ID
    WHERE P.COLLECTION_ID IN (SELECT VALUE FROM OPENJSON(@COLLECTIONS_CHOSEN,'$'))
        AND P.ID IN ( SELECT POST_ID FROM INTERACTIONS WHERE OWNERID=@UID AND SCOPE='VIEW'
    ) ---- VIEWED POST ^
    UNION
    SELECT
    P.ID,P.AUTHOR_ID,P.TITLE,P.COMMENT,P.DC,P.DM,P.LATITUDE,P.LONGITUDE,P.VISIBILITY_AREA_KM,P.EXCL_DATE_START,P.EXCL_DATE_END,P.RECURRENT,P.COLLECTION_ID,P.VIEWERS, P.order_id, P.post_id, C.COLOR, C.ICON,C.TITLE AS CATEGORY_NAME
        FROM NEXTPOST P
        INNER JOIN COLLECTIONS C ON C.ID = P.COLLECTION_ID
    UNION --- UNION WITH POST WITHOUT SEQUENCE
    SELECT
    P.ID,P.AUTHOR_ID,P.TITLE,P.COMMENT,P.DC,P.DM,P.LATITUDE,P.LONGITUDE,P.VISIBILITY_AREA_KM,P.EXCL_DATE_START,P.EXCL_DATE_END,P.RECURRENT,P.COLLECTION_ID,P.VIEWERS, NULL AS order_id, NULL AS post_id, C.COLOR, C.ICON,C.TITLE AS CATEGORY_NAME
    FROM POSTS P
    INNER JOIN COLLECTIONS C ON C.ID = P.COLLECTION_ID AND ( --COLLECTION TIME LIMITING CHECK
        
        (C.RECURRENT IS NULL  and (GETDATE()> ISNULL(C.EXCL_DATE_START,DATEADD(DAY,-1,GETDATE())) OR GETDATE() < ISNULL(C.EXCL_DATE_END,DATEADD(DAY,1,GETDATE()))) --IF NULL, MAKE IT FROM YESTERDAY TILL TOMORROW
        ) -- NO C.RECURRENTCY
        OR 
        (C.RECURRENT =1 AND (DAY(GETDATE()) BETWEEN DAY(C.EXCL_DATE_START) AND DAY(C.EXCL_DATE_START))) --EACH DAYS, IGNORING MONTHS
        OR
        (C.RECURRENT = 2 AND 
	        (DATEFROMPARTS(2000, MONTH(GETDATE()), DAY(GETDATE()))
	        BETWEEN DATEFROMPARTS(2000, MONTH(C.EXCL_DATE_START), DAY(C.EXCL_DATE_START))
		        AND DATEFROMPARTS(2000, MONTH(C.EXCL_DATE_END), DAY(C.EXCL_DATE_END)))
        ) -- EVERY YEAR, SAME DAYS SAME MONTH
    
    )
    WHERE
    C.SEQUENTIALS IS NULL -- NO SEQUENTIALITY INDICATED (GENERAL  POSTS)
    AND
    (
        P.VIEWERS IS NULL OR P.VIEWERS='[]' OR
         @UID IN (SELECT VALUE FROM OPENJSON(P.VIEWERS,'$'))
        AND
        (
			(P.RECURRENT IS NULL  and (GETDATE()> ISNULL(P.EXCL_DATE_START,DATEADD(DAY,-1,GETDATE())) OR GETDATE() < ISNULL(P.EXCL_DATE_END,DATEADD(DAY,1,GETDATE()))) --IF NULL, MAKE IT FROM YESTERDAY TILL TOMORROW
            ) -- NO P.RECURRENTCY
            OR 
            (P.RECURRENT =1 AND (DAY(GETDATE()) BETWEEN DAY(P.EXCL_DATE_START) AND DAY(P.EXCL_DATE_START))) --EACH DAYS, IGNORING MONTHS
            OR
            (P.RECURRENT = 2 AND 
	            (DATEFROMPARTS(2000, MONTH(GETDATE()), DAY(GETDATE()))
	            BETWEEN DATEFROMPARTS(2000, MONTH(P.EXCL_DATE_START), DAY(P.EXCL_DATE_START))
		            AND DATEFROMPARTS(2000, MONTH(P.EXCL_DATE_END), DAY(P.EXCL_DATE_END)))
            ) -- EVERY YEAR, SAME DAYS SAME MONTH
		)
        AND (
            @UID IN ( --CHECK IF THE USER REQUESTING IS ENABLED TO THAT CATEGORY
                SELECT VALUE FROM OPENJSON(C.VIEWERS,'$')
            ) OR C.VIEWERS='[]'
        )
    )
    AND C.ID IN (SELECT VALUE FROM OPENJSON(@COLLECTIONS_CHOSEN,'$'))

    -- NB: THE VISIBILITY_AREA_KM FILTER IS MADE ON BACKEND OF THE SERVER
END
GO
/****** Object:  StoredProcedure [dbo].[POST_MERGE]    Script Date: 06/05/2026 09:41:47 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[POST_MERGE]
    @POST_CONTENT NVARCHAR(MAX)
AS
BEGIN
    BEGIN TRANSACTION T1
    BEGIN TRY

          DECLARE @Result TABLE (
            ID INT
        );

        MERGE INTO POSTS AS T
        USING (
            SELECT 
                JJ.ID,
                JJ.AUTHOR_ID,
                JJ.TITLE,
                JJ.COMMENT,
                JJ.LATITUDE,
                JJ.LONGITUDE,
                JJ.ALTITUDE,
                JJ.EXCLUSIVITY,
                JJ.VISIBILITY_AREA_KM,
                DATE_EXL.DATE_START,
                DATE_EXL.DATE_END,
                JJ.VIEWERS,
                DATE_EXL.RECURRENT,
                JJ.COLLECTION_ID
            FROM OPENJSON(@POST_CONTENT,'$') WITH(
                ID INT,
                AUTHOR_ID INT,
                TITLE VARCHAR(50),
                COMMENT VARCHAR(MAX),
                LATITUDE FLOAT,
                LONGITUDE FLOAT,
                ALTITUDE FLOAT,
                VISIBILITY_AREA_KM INT,
                COLLECTION_ID INT,
                EXCLUSIVITY NVARCHAR(MAX) AS JSON,
                viewers NVARCHAR(MAX) '$.EXCLUSIVITY.viewers' AS JSON 
            ) JJ OUTER APPLY OPENJSON(EXCLUSIVITY,'$.daterange')WITH(
                DATE_START DATETIME,
                DATE_END DATETIME,
                RECURRENT INT
            ) DATE_EXL
            WHERE TITLE IS NOT NULL

        ) AS S
        ON T.ID = S.ID

        WHEN MATCHED THEN
            UPDATE SET
                    T.TITLE = ISNULL(S.TITLE,T.TITLE),
                    T.COMMENT = ISNULL(S.COMMENT,T.COMMENT),
                    T.DM = GETDATE(),
                    T.LATITUDE = ISNULL(S.LATITUDE,T.LATITUDE),
                    T.LONGITUDE = ISNULL(S.LONGITUDE,T.LONGITUDE),
                    T.ALTITUDE = ISNULL(S.ALTITUDE,T.ALTITUDE),
                    T.VISIBILITY_AREA_KM = ISNULL(S.VISIBILITY_AREA_KM,T.VISIBILITY_AREA_KM),
                    T.EXCL_DATE_START = ISNULL(S.DATE_START,T.EXCL_DATE_START),
                    T.EXCL_DATE_END = ISNULL(S.DATE_END,T.EXCL_DATE_END),
                    T.VIEWERS = ISNULL(S.VIEWERS,T.VIEWERS),
                    T.RECURRENT = ISNULL(S.RECURRENT,T.RECURRENT),
                    T.COLLECTION_ID = ISNULL(S.COLLECTION_ID,T.COLLECTION_ID)



        WHEN NOT MATCHED THEN
            INSERT (
                AUTHOR_ID, TITLE, COMMENT, DC, DM,
                LATITUDE, LONGITUDE, ALTITUDE,
                VISIBILITY_AREA_KM,
                EXCL_DATE_START,
                EXCL_DATE_END,
                VIEWERS,RECURRENT,
                COLLECTION_ID
                
            )
            VALUES (
                 S.AUTHOR_ID, S.TITLE, S.COMMENT, GETDATE(), GETDATE(),
                S.LATITUDE, S.LONGITUDE, S.ALTITUDE,
                S.VISIBILITY_AREA_KM,
                S.DATE_START,
                S.DATE_END,
                S.VIEWERS,
                S.RECURRENT,
                S.COLLECTION_ID
            )

        OUTPUT inserted.ID
        INTO @Result;

        COMMIT TRANSACTION T1;
        SELECT ID AS ID, 1 AS OK FROM @Result;
    END TRY
    BEGIN CATCH
        ROLLBACK TRANSACTION T1;
        PRINT ERROR_MESSAGE()
        SELECT -1 AS ID, 0 AS OK, ERROR_MESSAGE() AS MSG
    END CATCH
END;


GO
/****** Object:  StoredProcedure [dbo].[PROFILE_EditInfo]    Script Date: 06/05/2026 09:41:47 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[PROFILE_EditInfo]
@JSON NVARCHAR(MAX)
AS
BEGIN
	UPDATE U
	SET
	 U.USERNAME  = ISNULL(JJ.USERNAME , U.USERNAME)
	,U.PASSWORD  = ISNULL(JJ.PASSWORD ,U.PASSWORD)
	,U.EMAIL  = ISNULL(JJ.EMAIL ,U.EMAIL)
	,U.NAME  = ISNULL(JJ.NAME ,U.NAME)
	,U.SURNAME  = ISNULL(JJ.SURNAME ,U.SURNAME)
	,U.PROFILE_PICTURE  = ISNULL(JJ.PROFILE_PICTURE ,U.PROFILE_PICTURE)
	,U.DM = GETDATE()
	FROM USERS U
	INNER JOIN OPENJSON(@JSON,'$') WITH(
		UID INT
		,USERNAME VARCHAR(50)
		,PASSWORD NVARCHAR(512)
		,EMAIL VARCHAR(128)
		,NAME VARCHAR(30)
		,SURNAME VARCHAR(30)
		,PROFILE_PICTURE NVARCHAR(MAX)
	) JJ ON JJ.UID = U.ID

	SELECT 
	ID AS UID,
	USERNAME
	,PASSWORD
	,EMAIL
	,NAME
	,SURNAME
	 FROM USERS WHERE ID = (
		SELECT JSON_VALUE(@JSON,'$.UID')
	)

END

GO
/****** Object:  StoredProcedure [dbo].[PROFILE_GETINFO]    Script Date: 06/05/2026 09:41:47 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[PROFILE_GETINFO]
@JSON NVARCHAR(MAX)
AS
BEGIN
    SELECT U.USERNAME,U.NAME,U.SURNAME,U.DC
    FROM USERS U
    WHERE (u.ID= JSON_VALUE(@JSON,'$.uid') OR USERNAME=JSON_VALUE(@JSON,'$.username'))
    
END

GO
/****** Object:  StoredProcedure [dbo].[PROFILE_GetPFP]    Script Date: 06/05/2026 09:41:47 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[PROFILE_GetPFP]
@JSON NVARCHAR(MAX)
AS
BEGIN
	SELECT
	PROFILE_PICTURE
	FROM DBO.USERS
	WHERE USERNAME = JSON_VALUE(@JSON, '$.USERNAME') OR ID =  TRY_CAST(JSON_VALUE(@JSON, '$.USERNAME') AS INT)

END


GO
/****** Object:  StoredProcedure [dbo].[PROFILE_GETSTATS_CATEGORIES]    Script Date: 06/05/2026 09:41:47 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[PROFILE_GETSTATS_CATEGORIES]
@JSON NVARCHAR(MAX)
AS
BEGIN
    SELECT
    COUNT(P.ID) AS TOT_POSTS,
    C.COLOR AS COLLECTION_COLOR,
    C.ICON AS COLLECTION_ICON,
    C.[TITLE] AS COLLECTION_TITLE
    FROM USERS U
    LEFT JOIN POSTS P ON P.AUTHOR_ID = U.ID
    INNER JOIN COLLECTIONS C ON C.ID = P.COLLECTION_ID
    WHERE (u.ID= JSON_VALUE(@JSON,'$.uid') OR USERNAME=JSON_VALUE(@JSON,'$.username'))

    GROUP BY C.COLOR,C.ICON,C.[TITLE]
        
END
GO
/****** Object:  StoredProcedure [dbo].[PROFILE_GETSTATS_TIMEMONTHS]    Script Date: 06/05/2026 09:41:47 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[PROFILE_GETSTATS_TIMEMONTHS]
@JSON NVARCHAR(max)
AS
BEGIN
    SELECT
    COUNT(P.ID) AS TOT_POSTS,
    MONTH(P.DC) AS M
    FROM USERS U
    LEFT JOIN POSTS P ON P.AUTHOR_ID = U.ID
    INNER JOIN COLLECTIONS C ON C.ID = P.COLLECTION_ID
    WHERE (u.ID= JSON_VALUE(@JSON,'$.uid') OR USERNAME=JSON_VALUE(@JSON,'$.username'))

    GROUP BY MONTH(P.DC)
        
END

GO
/****** Object:  StoredProcedure [dbo].[PROFILE_INTEREST_GET]    Script Date: 06/05/2026 09:41:47 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

CREATE PROCEDURE [dbo].[PROFILE_INTEREST_GET]
@UID INT
AS
BEGIN
    
    SELECT * FROM INTERESTS I
    INNER JOIN HASHTAGS HT ON HT.ID=I.EXTERNAL_REF AND I.ENTITY='HASHTAG' 
    WHERE USER_ID =@UID

END

GO
/****** Object:  StoredProcedure [dbo].[PROFILE_INTEREST_MERGE]    Script Date: 06/05/2026 09:41:47 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[PROFILE_INTEREST_MERGE]
@UID INT, @JSON NVARCHAR(MAX)
AS
BEGIN
    
    DECLARE @TABLE TABLE (
        VAL VARCHAR(200),
        ENTITY VARCHAR(200)
    );
    INSERT INTO @TABLE(VAL,ENTITY)
    SELECT value,entity FROM OPENJSON(@JSON,'$')WITH(
        value VARCHAR(200),
        entity VARCHAR(200)
    );

    

    DELETE FROM INTERESTS WHERE [USER_ID]=@UID
    AND NOT EXISTS (
        SELECT * FROM @TABLE T
        INNER JOIN HASHTAGS HT ON HT.TITLE = T.VAL AND T.ENTITY='HASHTAG'
        WHERE HT.ID=EXTERNAL_REF AND ENTITY = T.ENTITY
    )

    --- CREATE HASHTAG IF NOT EXISTS
    INSERT INTO HASHTAGS (TITLE,DC,UC)
    SELECT T.VAL, GETDATE(),@UID 
    FROM @TABLE T WHERE T.ENTITY='HASHTAG'
    AND NOT EXISTS(SELECT * FROM HASHTAGS WHERE TITLE = T.VAL )

    -- INSERT HASHTAG ON INTERESTS
    INSERT INTO INTERESTS ([USER_ID],EXTERNAL_REF,ENTITY)
    -- SELECT @UID, ISNULL(HT.ID,COLLETION.ID--> TO DO WITH CASE WHEN), T.ENTITY
    SELECT @UID, HT.ID, T.ENTITY
    FROM @TABLE T 
    INNER JOIN HASHTAGS HT ON HT.TITLE = T.VAL
    WHERE NOT EXISTS (
        SELECT * FROM INTERESTS WHERE ENTITY=T.ENTITY AND EXTERNAL_REF=HT.ID AND [USER_ID] = @UID
    )


    EXEC PROFILE_INTEREST_GET @UID=@UID

END


GO
/****** Object:  StoredProcedure [dbo].[REPORT_NEW]    Script Date: 06/05/2026 09:41:47 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

CREATE PROCEDURE [dbo].[REPORT_NEW]
@JSON NVARCHAR(MAX)
AS
BEGIN
	INSERT INTO REPORTS(USERID,MOTIVE,POSTID,DC,KIND,PROCESSED)
	SELECT uid, motive, postid, getdate(), kind, 0 FROM OPENJSON(@JSON,'$')WITH(
		postid int,
		uid int, 
		motive varchar(2400),
		kind varchar(40)
	) J
	WHERE NOT EXISTS( --JUST ONE REPORT FOR POST&USER :: A UNIQUE KEY WOULD DO THE SAME EFFECT BUT NAH
		SELECT * FROM REPORTS WHERE USERID=J.[uid] AND POSTID=J.postid
	)

END

GO
/****** Object:  StoredProcedure [dbo].[USERS_LIST]    Script Date: 06/05/2026 09:41:47 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[USERS_LIST]
@JSON NVARCHAR(MAX)=NULL
AS
BEGIN
	SELECT ID AS UID , USERNAME,NAME,SURNAME, PROFILE_PICTURE FROM USERS
	WHERE EMAIL_VERIFIED=1 AND ID<>  JSON_VALUE(@JSON,'$.uid')
END


GO
USE [master]
GO
ALTER DATABASE [GEOMEDIA] SET  READ_WRITE 
GO
