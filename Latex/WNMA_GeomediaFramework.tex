...
To ensure that the GeoMedia app functions as intended, there is the need to design an opportune architecture, even if not strictly related with the mobile application itself.
In fact, the adopted three-tier architecture makes the app work as intended, as shown in Figure 1:
\begin{itemize}
    \item Client layer, represented by mobile phones since GeoMedia is the Android app that serve as interaction point with the users and making HTTP requests to server side
    \item Application layer, where the HTTP server takes place, by handling the requests received with the opportune functions and logic; for example compute the distance from the given location to the exclusive posts.
    \item Data layer, represented by the DBMS (DataBase Management System) that store the data in the opportune data structures and retrieve it when asked.
\end{itemize}
...
\begin{itemize}
    \item Profile, in this tab users can see their posts as list and open them; or change the server configuration parameters (i.e. IP Address or HTTP port)
    \item  Map, this section is the heart of the app, by offering a navigation through a geographical map to look for the comments and posts uploaded by other users. Furthermore this tab provide also the access to the publish component that allows users to create a new post based on their coordinates.
\end{itemize}
...
\\
The post is composed of some attributes:
\begin{itemize}
    \item Title
    \item Comment
...
To provide a geographical map has been implemented inside GeoMedia the Pigeon Map\cite{b8} provided as a React Component, that render the graphical map and allows to put some ``markers" on it.
\\
TODO
Each posts published will be in the map as marker, there are four type of 
provided from the HTTP server following a client request, will be rendered
